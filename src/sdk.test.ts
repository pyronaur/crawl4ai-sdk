...
describe('Crawl4AI SDK', () => {
  ...
  describe('Crawl Method', () => {
    test('should validate URLs', async () => {
      ...
      await expect(client.crawl({ urls: '' })).rejects.toThrow(RequestValidationError);
      await expect(client.crawl({ urls: '://malformed' })).rejects.toThrow(RequestValidationError);
  ...
  describe('Markdown Method', () => {
    test('should validate URL', async () => {
      await expect(client.markdown({ url: 'invalid-url' })).rejects.toThrow(RequestValidationError);
  ...
  describe('HTML Method', () => {
    test('should validate URL', async () => {
      await expect(client.html({ url: 'invalid-url' })).rejects.toThrow(RequestValidationError);
      await expect(client.html({ url: '' })).rejects.toThrow(RequestValidationError);
      await expect(client.html({ url: '://malformed' })).rejects.toThrow(RequestValidationError);
    });
  ...
  describe('LLM Method', () => {
    test('should validate URL', async () => {
      await expect(client.llm('invalid-url', 'test query')).rejects.toThrow(RequestValidationError);
    ...
    test('should accept valid URL and query', async () => {
      const promise = client.llm('https://example.com', 'What is the main content of this page?');
      expect(promise).toBeInstanceOf(Promise);
      promise.catch(() => {}); // Prevent unhandled rejection
    });
